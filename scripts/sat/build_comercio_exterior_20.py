#!/usr/bin/env python3
"""Build a reproducible SAT Comercio Exterior 2.0 SQLite dataset.

SAT remains authoritative for the Comercio Exterior complement and its
catalogs. PhpCfdi's versioned ``resources-sat-catalogs`` SQLite release is used
as a technical ingestion mirror. Only the exact CCE 2.0-owned table family is
copied to the output artifact.

CCE 2.0 also reuses four CFDI 4.0 catalogs: country, currency, postal code and
tax regime. Those tables are validated in the source mirror but deliberately
not duplicated into this artifact. The manifest declares the dependency on
``sat.cfdi_4`` explicitly so dataset profiles/resolvers can fetch both datasets.

The builder fails closed when the CCE 2.0 table family or required shared CFDI
dependencies change and emits both a file SHA-256 and a semantic content
SHA-256. The generic catalog-maintenance workflow can therefore publish a new
data release only when CCE-owned content actually changes.
"""
...
AUTHORITATIVE_TECHNICAL_PAGE = (
    "http://omawww.sat.gob.mx/tramitesyservicios/Paginas/"
    "complemento_comercio_exterior.htm"
)
...
# CCE 2.0 reuses these CFDI 4.0 catalogs instead of owning duplicate tables.
DEPENDENCY_TABLES = (
...
def fetch_latest_release() -> dict[str, Any]:
    ...
    request = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "catalogmx-comercio-exterior-20-builder",
        },
    )
...
def download(url: str, destination: Path) -> None:
    ...
    request = urllib.request.Request(
        url, headers={"User-Agent": "catalogmx-comercio-exterior-20-builder"}
    )
...
def discover_tables(connection: sqlite3.Connection) -> tuple[str, ...]:
    """Return and validate the complete CCE 2.0-owned table set."""
    rows = connection.execute(
        "SELECT name FROM sqlite_master "
        "WHERE type = 'table' AND name LIKE 'cce_20_%' ORDER BY name"
    ).fetchall()
    ...
    if tables != expected:
        ...
        raise RuntimeError(
            "unexpected Comercio Exterior 2.0 schema; "
            f"missing={missing or 'none'}, extra={extra or 'none'}"
        )
    return tables


def validate_dependencies(connection: sqlite3.Connection) -> None:
    """Require every CFDI 4.0 catalog that CCE 2.0 reuses."""
    existing = {
        row[0]
    ...
    missing = sorted(set(DEPENDENCY_TABLES) - existing)
    if missing:
        raise RuntimeError(
            "missing Comercio Exterior 2.0 CFDI dependencies: " + ", ".join(missing)
        )


def copy_subset(source_db: Path, output_db: Path) -> dict[str, int]:
    """Copy only CCE 2.0-owned tables and indexes from the mirror database."""
...
def build_manifest(
    ...
    """Create deterministic provenance, dependency and integrity metadata."""
    return {
        "schema_version": 1,
        ...
        "authority": {
            ...
            "technical_page": AUTHORITATIVE_TECHNICAL_PAGE,
        },
        "ingestion": {
...
def build_latest(output_dir: Path) -> tuple[Path, Path, dict[str, Any]]:
    """Download the latest mirror release and build the CCE 2.0 dataset."""
    metadata = fetch_latest_release()
    with tempfile.TemporaryDirectory(prefix="catalogmx-cce-20-") as temp_dir:
        temp = Path(temp_dir)
