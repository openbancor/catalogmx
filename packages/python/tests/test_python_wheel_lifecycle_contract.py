...
PACKAGE_DYNAMIC_DB = (
    ROOT / "packages" / "python" / "catalogmx" / "data" / "mexico_dynamic.sqlite3"
)
PACKAGE_CLAVE_PROD_SERV_DB = (
    ROOT / "packages" / "python" / "catalogmx" / "data" / "clave_prod_serv.db"
)
CANONICAL_CLAVE_PROD_SERV_DB = (
    ROOT / "packages" / "shared-data" / "sqlite" / "clave_prod_serv.db"
)
...
def test_clave_prod_serv_is_immutable_package_data() -> None:
    assert (
        PACKAGE_CLAVE_PROD_SERV_DB.read_bytes()
        == CANONICAL_CLAVE_PROD_SERV_DB.read_bytes()
    )
    assert '"data/*.db"' in PYPROJECT.read_text(encoding="utf-8")
