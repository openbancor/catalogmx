...
class TestComercioExteriorCatalogMethods:
    ...
    def test_moneda_validate_conversion_all_paths(self):
        ...
        result1 = MonedaCatalog.validate_conversion_usd(
            {"moneda": "MXN", "total": 20000, "tipo_cambio_usd": None, "total_usd": 1000}  # Missing
        )
        assert len(result1.get("errors", [])) > 0

        # Line 72: USD but total != total_usd
        result2 = MonedaCatalog.validate_conversion_usd(
            {"moneda": "USD", "total": 100, "tipo_cambio_usd": 1.0, "total_usd": 99}  # Different
        )
...
class TestRFCGeneratorAllLines:
    ...
    def test_fisica_with_compound_names(self):
        ...
        rfc1 = RFCGeneratorFisicas(
            nombre="José María", paterno="García", materno="López", fecha=date(1990, 1, 1)
        )
        assert len(rfc1.rfc) == 13

        # Test María (compound name)
        rfc2 = RFCGeneratorFisicas(
            nombre="María Elena", paterno="Pérez", materno="González", fecha=date(1990, 1, 1)
        )
    ...
    def test_fisica_with_various_names(self):
        ...
        # Test normal names
        rfc1 = RFCGeneratorFisicas(
            nombre="Juan", paterno="Perez", materno="Garcia", fecha=date(1990, 1, 1)
        )
        assert len(rfc1.rfc) == 13

        # Test with ñ
        rfc2 = RFCGeneratorFisicas(
            nombre="Juan", paterno="Peña", materno="Muñoz", fecha=date(1990, 1, 1)
        )
        assert len(rfc2.rfc) == 13
    ...
    def test_moral_with_various_companies(self):
        ...
        # Test normal company
        rfc1 = RFCGeneratorMorales(
            razon_social="Tecnologia Sistemas Integrales S.A.", fecha=date(2009, 9, 9)
        )
