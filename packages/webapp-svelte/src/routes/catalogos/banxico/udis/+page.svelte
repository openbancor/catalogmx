<script lang="ts">
	import { ChevronRight, TrendingUp, Coins } from 'lucide-svelte';
	...
	onMount(async () => {
		try {
			const res = await fetch('/data/banxico/udi.json');
			data = await res.json();
		} catch (error) {
			...
			<div class="flex items-center justify-between">
				<div>
					...
					<p class="text-4xl font-bold text-green-700 dark:text-green-300 mt-1">
						${data.values[0].value.toFixed(6)} MXN
					</p>
					...
					<tbody class="divide-y divide-slate-200 dark:divide-slate-700">
						{#each displayedValues as item}
							<tr class="hover:bg-slate-50 dark:hover:bg-slate-800/50">
								...
								<td class="py-3 px-4 text-right font-mono text-slate-900 dark:text-white">${item.value.toFixed(6)}</td>
							</tr>
						{/each}
					</tbody>
				</table>
