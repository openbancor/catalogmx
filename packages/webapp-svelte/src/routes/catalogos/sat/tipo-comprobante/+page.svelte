<script lang="ts">
	...
	async function loadData() {
		try {
			...
			const response = await fetch('/data/sat/cfdi_4.0/c_TipoDeComprobante.json');
			if (!response.ok) {
...
<svelte:head>
	...
	<meta name="description" content="Catálogo de tipos de comprobante del SAT para CFDI 4.0." />
</svelte:head>
		...
		<div class="flex items-start gap-4">
			...
			<div>
				...
				<p class="text-slate-600 dark:text-slate-300">
					Tipos de comprobante válidos para facturas electrónicas (CFDI 4.0)
				</p>
			</div>
		</div>
	</div>
		...
		<div class="mt-8 card p-6">
			...
			<div class="space-y-2 text-sm text-slate-600 dark:text-slate-300">
				<p>
					...
					de operación o documento que se está emitiendo.
				</p>
				<p>
				...
				<ul class="list-disc list-inside ml-4 space-y-1">
					...
					<li><strong>E - Egreso:</strong> Nota de crédito (devoluciones, descuentos, bonificaciones)</li>
					<li><strong>T - Traslado:</strong> Comprobante de traslado de mercancías (sin valor fiscal)</li>
					<li><strong>N - Nómina:</strong> Recibo de pago de nómina</li>
					<li><strong>P - Pago:</strong> Recibo electrónico de pago (complemento de pago)</li>
				</ul>
				<p>
				...
				<p>
					...
					Cada tipo tiene reglas específicas de validación y complementos permitidos.
				</p>
			</div>
		</div>
	{/if}
