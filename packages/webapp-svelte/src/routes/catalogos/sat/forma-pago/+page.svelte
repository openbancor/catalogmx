<script lang="ts">
	...
	async function loadData() {
		try {
			...
			const response = await fetch('/data/sat/cfdi_4.0/c_FormaPago.json');
			if (!response.ok) {
...
<svelte:head>
	...
	<meta name="description" content="Catálogo de formas de pago del SAT para CFDI 4.0." />
</svelte:head>
		...
		<div class="flex items-start gap-4">
			...
			<div>
				...
				<p class="text-slate-600 dark:text-slate-300">
					Formas de pago válidas para facturas electrónicas (CFDI 4.0)
				</p>
			</div>
		</div>
	</div>
		...
		<div class="mt-8 card p-6">
			...
			<div class="space-y-2 text-sm text-slate-600 dark:text-slate-300">
				<p>
					...
					se realiza o realizará el pago de la operación (efectivo, transferencia, tarjeta, etc.).
				</p>
				<p>
					<strong>Fuente:</strong> Servicio de Administración Tributaria (SAT)
				</p>
				<p>
					<strong>Uso:</strong> Campo obligatorio en el CFDI 4.0 cuando el Método de Pago es "PUE" (Pago en una sola exhibición).
				</p>
			</div>
		</div>
	{/if}
