...
export default function Layout() {
  ...
  const getCurrentPageId = (): string => {
    ...
    if (path.startsWith('validators')) return 'validators';
    if (path.startsWith('calculators')) return 'calculators';
    if (path.startsWith('catalogs/')) {
      const datasetId = path.replace('catalogs/', '');
  ...
  const currentNavItem = flatNavigation.find(i => i.id === currentPageId);
  ...
  return (
    <div className="flex min-h-screen bg-background text-foreground">
                  ...
                  <div key={category.id} className="mb-4">
                    ...
                    <div className="space-y-0.5 mt-1">
                      {catalogs.slice(0, 5).map((cat) => (
                        <button
                          key={cat.id}
                          onClick={() => handleNavigate(`/catalogs/${cat.id}`)}
                          className={cn(
                            "w-full rounded-lg px-3 py-1.5 text-left text-xs font-medium hover:bg-muted truncate transition-colors",
                            currentPageId === `dataset-${cat.id}` ? "bg-primary/10 text-primary" : "text-muted-foreground"
                          )}
                        >
                          {cat.label}
                        </button>
        ...
        <header className="sticky top-0 z-30 flex h-16 items-center gap-4 border-b border-border bg-background/95 px-4 backdrop-blur supports-[backdrop-filter]:bg-background/80">
          ...
          <div className="flex flex-col min-w-0">
            ...
            <span className="text-base font-semibold truncate">
               {currentNavItem ? t(currentNavItem.label) : (currentPageId.startsWith('dataset') ? 'Catálogo' : '')}
            </span>
          </div>
      ...
      <MobileNav
        ...
        onNavigate={(pageId) => {
          const navItem = flatNavigation.find(i => i.id === pageId);
          if (navItem) {
              ...
              <div className="divide-y rounded border">
                {filteredCatalogs.map((cat) => (
                  <button
                    ...
                    onClick={() => {
                      handleNavigate(`/catalogs/${cat.id}`);
