...
from banxico_sqlite_helper import EXIT_ERROR, EXIT_NO_OBSERVATION, EXIT_SUCCESS
...
def fetch_udi_data(token: str, start_date: str, end_date: str) -> list[dict[str, Any]]:
    ...
    try:
        ...
        if e.code == 401:
            raise ValueError(
                "Invalid Banxico API token. Get one at "
                "https://www.banxico.org.mx/SieAPIRest/service/v1/token"
            )
        if e.code == 429:
            raise ValueError(
                "Rate limit exceeded. The script already implements delays, "
                "but Banxico may have additional limits."
            )
...
def ensure_database_exists(db_path: Path):
    ...
    if not db_path.exists():
        ...
        if schema_path.exists():
            ...
            db.close()
            print("[fetch] ✓ Database created with schema")
        else:
            print("[fetch] Warning: schema_dynamic.sql not found, creating minimal schema")
            db = sqlite3.connect(db_path)
            db.execute(
                """
...
def main():
    ...
    if not args.token:
        ...
        print("  export BANXICO_TOKEN='your_token_here'")
        print("  ./scripts/full_check.sh")
        print("")
    ...
    if not start_date:
        if args.full:
            ...
            if last_date:
                ...
                print(
                    f"[fetch] Incremental mode: last record is {last_date}, "
    ...
    if start_date_obj > end_date_obj:
        last_date = get_last_date_from_db(args.database)
        print("[fetch] ✓ Data is already up to date")
    ...
    try:
        ...
        print(
            f"[fetch] Latest UDI: {new_records[-1]['valor']} MXN "
            f"({new_records[-1]['fecha']})"
        )
