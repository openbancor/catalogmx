...
describe('API key authentication', () => {
  ...
  test.each([JSON.stringify({}), JSON.stringify([])])(
    'fails closed when the key secret is not a non-empty array: %s',
    async (secret) => {
      await expect(
        authenticateRequest(request('test-api-key'), env({ CATALOGMX_API_KEYS: secret }))
      ).rejects.toMatchObject({ status: 503, code: 'configuration_error' });
  ...
  test.each([
    ...
    [{ id: 'missing-digest', active: true }],
    [{ id: 'bad-digest', active: true, digest: 'not-a-sha256-digest' }],
  ])('fails closed for an invalid key record: %j', async (record) => {
    await expect(
      authenticateRequest(
        request('test-api-key'),
        env({ CATALOGMX_API_KEYS: JSON.stringify([record]) })
      )
    ).rejects.toMatchObject({ status: 503, code: 'configuration_error' });
  });
});
