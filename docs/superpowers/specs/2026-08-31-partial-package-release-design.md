...
- Existing registry checks remain authoritative: an existing version with different or malformed bytes fails, while an exact match is skipped. New PyPI, npm, and pub.dev uploads are polled and rechecked against the verified artifacts before the release job can proceed.
