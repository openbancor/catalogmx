...
`RegistroIdentTribCatalog` no longer treats the tracked list of 15 generic identification labels as a regulatory catalog. Those codes remain small code-owned `catalogmx-legacy` aliases so existing `get_tipo()`, `is_valid()` and `validate_tax_id()` callers do not break, but they are not used to determine the legal format of `NumRegIdTrib`. SAT publishes that rule on `cfdi_40_paises`; `get_country_rule()` and `validate_for_country()` therefore resolve the shared `sat.cfdi_4` artifact and validate by Alpha-3 country. Current authority data publishes a local identity pattern for USA, CAN and MEX only. USA and Canada use a nine-digit regex; countries without a published pattern are not subjected to a fabricated local regex. Mexico additionally declares `validacion_identidad_tributaria = "Lista del SAT"`, so the API reports `requires_external_validation=True` instead of claiming that local pattern matching is sufficient. `ComercioExteriorValidator` now applies these country-scoped rules to `receptor.NumRegIdTrib` even when the obsolete `tipo_registro_trib` convenience is absent.

Several historical APIs deliberately remain outside this slice because their source boundary is different. `ClavePedimentoCatalog` exposes roughly 42 general Anexo 22 customs keys, while the CCE 2.0-owned `cce_20_claves_pedimentos` table contains only `A1`; it needs its own customs-source lifecycle rather than destructive narrowing. `PaisCatalog` also depends on CFDI authority data, but its historical Alpha-2 lookup is an ISO 3166 enrichment that `cfdi_40_paises` does not publish. CatalogMX does not add a new runtime dependency or hidden Alpha-2 mapping merely to preserve that convenience; country cutover remains deferred until an explicit ISO enrichment/source policy is defined.

Installed-wheel CI builds a verified local `sat.comercio_exterior` release and exercises the four CCE-owned migrated public classes through the release-pointer/cache path without packaging the tracked Comercio Exterior JSON directory. The existing `Python CFDI Consumer Data` gate separately exercises `MonedaCatalog` and country-scoped tax-identity validation through the verified `sat.cfdi_4` release/cache path, matching the true data owner instead of creating a duplicate CCE lifecycle.
