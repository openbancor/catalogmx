...
Channels created by the pre-pointer CatalogMX maintenance workflow are migrated only after strict verification. A legacy channel must carry the known old CatalogMX publication-note prefix, exactly the expected artifact and manifest assets, a schema-v1 manifest for the same dataset/version and artifact name, and an artifact whose SHA-256 matches that manifest. The new immutable release is then created and verified first; the stable pointer is committed second; only after the pointer is current are the old channel assets deleted. Any unknown prose body, unexpected asset set, manifest mismatch, or checksum mismatch fails closed instead of being auto-migrated.
